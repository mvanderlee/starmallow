...
from starmallow.responses import ORJSONResponse
...
############################################################
# region
@app.get("/path/str", response_class=ORJSONResponse)
def get_path_str() -> str:
    return 5


@app.get("/path/int", response_class=ORJSONResponse)
def get_path_int() -> int:
    return "5"


@app.get("/path/bool", response_class=ORJSONResponse)
def get_path_bool() -> bool:
    return True


@app.get("/path/date", response_class=ORJSONResponse)
def get_path_date() -> dt.date:
    return dt.date(2023, 2, 27)


@app.get("/path/datetime", response_class=ORJSONResponse)
def get_path_datetime() -> dt.datetime:
    return dt.datetime(2023, 2, 27, 11, 37, 43)


@app.get("/path/time", response_class=ORJSONResponse)
def get_path_time() -> dt.time:
    return dt.time(11, 37, 43)


@app.get("/path/timedelta", response_class=ORJSONResponse)
def get_path_timedelta() -> dt.timedelta:
    return dt.timedelta(days=1, hours=6, seconds=321)


@app.get("/path/uuid", response_class=ORJSONResponse)
def get_path_uuid() -> UUID:
    return UUID(hex='2c55d0f62629450a8eee71f791cd8eb9')


@app.get("/path/decimal", response_class=ORJSONResponse)
def get_path_decimal() -> Decimal:
    return Decimal(123456789)


@app.get("/path/enum", response_class=ORJSONResponse)
def get_path_enum() -> MyEnum:
    return MyEnum.optionA


@app.get("/path/literal", response_class=ORJSONResponse)
...
@app.get("/path/final", response_class=ORJSONResponse)
def get_path_final() -> FinalItem:
    return FinalItem()


@app.get("/path/date_field", response_class=ORJSONResponse)
def get_path_date_field() -> mf.Date:
    return dt.date(2023, 2, 27)


@app.get("/path/date_model", response_model=mf.Date(), response_class=ORJSONResponse)
def get_path_date_model():
    return dt.date(2023, 2, 27)


@app.get("/path/dataclass", response_class=ORJSONResponse)
def get_path_dataclass() -> Item:
    return {'item_id': dt.date(2023, 2, 27)}


@app.get("/path/schema", response_class=ORJSONResponse)
def get_path_schema() -> Item.Schema:
    return {'item_id': dt.date(2023, 2, 27)}


@app.get("/path/dataclass_model", response_model=Item, response_class=ORJSONResponse)
def get_path_dataclass_model():
    return {'item_id': dt.date(2023, 2, 27)}


@app.get("/path/schema_model", response_model=Item.Schema, response_class=ORJSONResponse)
...
@app.get("/path/custom_description", response_description='Custom Description', response_class=ORJSONResponse)
